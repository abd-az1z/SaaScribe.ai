...
function FileUploader() {
  ...
  const statusIcon: Record<StatusText, JSX.Element> = {
    [StatusText.UPLOADING]: (
      <RocketIcon className="animate-spin h-6 w-6 text-blue-500" />
    ),
    [StatusText.UPLOADED]: (
      <CheckCircleIcon className="h-6 w-6 text-green-500" />
    ),
    [StatusText.SAVING]: <SaveIcon className="h-6 w-6 text-yellow-500" />,
    [StatusText.GENERATING]: <HammerIcon className="h-6 w-6 text-gray-500" />,
  };
    ...
    <div className="flex flex-col">
      ...
      {uploadingProgress && status && (
        <div className="flex flex-col items-center justify-center gap-5 mt-10">
          <div
            className={`radial-progress bg-blue-300 text-gray-200 border-blue-700 border-2 ${
              progress === 100 ? "hidden" : ""
            }`}
            role="progressbar"
            style={{
          ...
          <p className="text-blue-500 animate-pulse">{String(status)}</p>
        </div>
      )}
      ...
      {!uploadingProgress && (
        <div
          {...getRootProps()}
          className={`p-10 border border-blue-400 ${
            isFocused || isDragActive ? "bg-blue-400" : "bg-gray-200"
          }  `}
        >
          <input {...getInputProps()} />
